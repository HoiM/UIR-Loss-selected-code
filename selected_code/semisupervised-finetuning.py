...
class AccMetric(mx.metric.EvalMetric):
  ...
  def update(self, labels, preds):
    ...
    labels=labels[0]
    #print(labels.stype)
    labels = nd.slice_axis(labels, axis=0, begin=0, end=args.per_batch_size-args.num_unlabeled)
    labels = [labels]
    #print(preds) 
    #print(labels)
...
class SoftmaxLossValueMetric(mx.metric.EvalMetric):
  def __init__(self):
    self.axis = 1
    super(SoftmaxLossValueMetric, self).__init__(
        'softmax-loss-value', axis=self.axis,
        output_names=None, label_names=None)
    self.losses = []
...
class LikelihoodLossValueMetric(mx.metric.EvalMetric):
  def __init__(self):
    self.axis = 1
    super(LikelihoodLossValueMetric, self).__init__(
        'likelihood-loss-value', axis=self.axis,
        output_names=None, label_names=None)
    self.losses = []
...
def parse_args():
  ...
  parser.add_argument('--prefix', default='../model/filtered/finetune-r100', help='directory to save model.')
  parser.add_argument('--pretrained', default='../pretrained/resnet/r100fc-with-fc7/model,174', help='pretrained model to load')
  ...
  parser.add_argument('--lr-steps', type=str, default='30000,60000,80000,100000', help='steps of lr changing')
  ...
  parser.add_argument('--target', type=str, default='val/lfw,val/cfp_fp,val/agedb_30', help='verification targets')

  # add new arguments
  parser.add_argument('--num-unlabeled', type=int, default=32, help='number of unlabeled samples per batch')
  parser.add_argument('--likelihood-scale-factor', type=float, default=0.1, help='loss weight for log likelihood loss')
  parser.add_argument('--unlabeled-dataset-prefix', type=str, default='../dataset/train_unknown/train_unknown', help='unlabeled dataset prefix')
...
def get_symbol(args, arg_params, aux_params):
  ...
  if args.loss_type==0: #softmax
    ...
    subtract =  args.num_classes * math.log(args.num_classes)############### This is calculated based on the number of labeled classes
    num_unlabeled = args.num_unlabeled
    num_labeled = args.per_batch_size - args.num_unlabeled
    per_batch_size = args.per_batch_size
    assert num_labeled > 0
    fc7_2 = mx.sym.slice_axis(fc7, axis=0, begin=num_labeled, end=per_batch_size)
    ...
    gt_label = mx.sym.slice_axis(gt_label, axis=0, begin=0, end=num_labeled)
    ############################################################
  ...
  if args.loss_type == 4:
      ...
      cos_fc7 = mx.symbol.BlockGrad(fc7) # TODO: maybe this softmax makes the accuracy inconsistent
      gt_label_one_hot = mx.sym.one_hot(gt_label, args.num_classes)
      fc7 = -mx.sym.log_softmax(fc7, axis=1)
      softmax = gt_label_one_hot*fc7
      softmax = mx.sym.sum(softmax) / (args.per_batch_size - args.num_unlabeled)
...
def train_net(args):
    ...
    path_imgrec = os.path.join(data_dir, "train_known/train_known.rec")
    ...
    train_dataiter = FaceImageIter(
        per_batch_size           = args.per_batch_size,
        num_unlabeled            = args.num_unlabeled,
        fake_label               = args.num_classes, # actually does not make much different
        unlabeled_dataset_prefix = args.unlabeled_dataset_prefix,
    ...
    _metric1 = AccMetric()
    _metric2 = LossValueMetric()
    _metric3 = SoftmaxLossValueMetric()
    _metric4 = LikelihoodLossValueMetric()
    eval_metrics = [mx.metric.create(_metric1), mx.metric.create(_metric2), mx.metric.create(_metric3), mx.metric.create(_metric4)]
